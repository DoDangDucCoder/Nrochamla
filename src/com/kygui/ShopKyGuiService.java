...
public class ShopKyGuiService {
    ...
    public void buyItem(Player pl, int id) {
        ...
        if (it.goldSell > 0) {
            ...
            if (pl.inventory.gem >= it.rubySell) {
                pl.inventory.gem -= it.rubySell;
                isBuy = true;
            } else {
    ...
    public void claimOrDel(Player pl, byte action, int id) {
        ...
        switch (action) {
            ...
            case 2: // nhận tiền
                ...
                if (it.goldSell > 0) {
                    if((pl.inventory.gold +( it.goldSell - it.goldSell * 5 / 100))<=pl.inventory.maxgold){
                        pl.inventory.gold += it.goldSell - it.goldSell * 5 / 100;
                    }else{
                        Service.getInstance().sendThongBao(pl, "Vượt quá số lượng vàng giới hạn.");
                    }
                    
                } else if (it.rubySell > 0) {
                    pl.inventory.gem += it.rubySell - it.rubySell * 5 / 100;
                }
                if (ShopKyGuiManager.gI().listItem.remove(it)) {
                    Service.getInstance().sendMoney(pl);
    ...
    public List<ItemKyGui> getItemCanKiGui(Player pl) {
        ...
        pl.inventory.itemsBag.stream().filter((it) -> (it.isNotNullItem() && 
               ((
                       it.template.type < 100 && it.template.type >= 0) //|| 
                       //it.template.type == 12 || it.template.type == 33 || 
                       //it.template.type == 29 || it.template.type == 27 || 
                       //it.template.type == 11 || it.template.type == 5
                ))).forEachOrdered((it) -> {    
    ...
    public void KiGui(Player pl, int id, int money, byte moneyType, int quantity) {
        System.out.println("lmao ky gui");
        try {
