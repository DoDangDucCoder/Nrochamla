...
public class Pet extends Player {
    ...
    private static final short[][] PET_ID = {{285, 286, 287}, {288, 289, 290}, {282, 283, 284}, {304, 305, 303}, {2116, 2117, 2118}, {2075, 2076, 2077}};
    ...
    private String getTextStatus(byte status) {
        switch (status) {
            case FOLLOW:
                return "Theo Theo CL Mệt Người";
            case PROTECT:
                return "Á..À!!Mày Dám Đánh Sư Phụ Tao À, Sư Phụ để Con";
            case ATTACK:
                return "Đánh Đánh Suốt Ngày Mệt VL, Cút";
            case GOHOME:
                return "Về Đây, Đừng Gọi Nữa Ngủ";
            default:
                return "";
        }
    }
    ...
    public void fusion3(boolean porata2) {
        if (this.isDie()) {
            Service.getInstance().sendThongBao(master, "Không thể thực hiện");
            return;
        }
    ...
    public void fusion4(boolean porata2) {
        if (this.isDie()) {
            Service.getInstance().sendThongBao(master, "Không thể thực hiện");
            return;
        }
    ...
    public void update() {
        try {
            ...
            if (justRevived && this.zone == master.zone) {
                Service.gI().chatJustForMe(master, this, "Con Xin Bố, Đã Yếu Cứ Bắt Đi Đánh Quái Mạnh");
                justRevived = false;
            }
