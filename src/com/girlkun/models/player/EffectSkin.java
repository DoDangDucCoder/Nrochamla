...
public class EffectSkin {
    ...
    public void update() {
       ...
       if (this.player.zone != null && !MapService.gI().isMapOffline(this.player.zone.map.mapId)) {
        if (!player.isBoss && !player.isNewPet ) {
          if (Util.canDoWithTime(lastTimehoada, Timehoada)) {
            EffectSkillService.gI().removehoada(player);
          }
        }
      }
    ...
    private void updateDraBulaHoaDa() {
        try {
            if (player.nPoint.hoada) {
                if (Util.canDoWithTime(lastTimehoada, 30000)) {
                    ...
                    for (Player pl : players) {
                        int timehoada = 5000;
                        EffectSkillService.gI().sethoada(pl, System.currentTimeMillis(), timehoada);
                        ItemTimeService.gI().sendItemTime(pl, 4392, timehoada/1000);
                    }
    ...
    private void updateCTHaiTac() {
        if (this.player.setClothes.ctHaiTac != -1
            ...
            for (Player pl : players) {
                ...
                if (!pl.isPet && !pl.isMiniPet && Util.canDoWithTime(lastTimeUpdateCTHT, 5000)) {
