...
//                Service.getInstance().releaseCooldownSkill(player);
    ...
    private void useSkillAttack(Player player, Player plTarget, Mob mobTarget) {
        ...
        switch (player.playerSkill.skillSelect.template.id) {
            ...
            case Skill.ANTOMIC:
                if (plTarget != null) {
                    playerAttackPlayer2(player, plTarget, miss);
                }
                if (mobTarget != null) {
                    playerAttackMob2(player, mobTarget, miss, false);
                }
                if (player.mobMe != null) {
                    player.mobMe.attack(plTarget, mobTarget);
                }
                affterUseSkill(player, player.playerSkill.skillSelect.template.id);
//                if (player.inventory != null && player.inventory.itemsBody.size() > 0 && player.inventory.itemsBody.get(5) != null) {
//                    Item item = player.inventory.itemsBody.get(5);
//                    if (item.itemOptions != null && item.itemOptions.size() > 0) {
//                        for (int i = 0; i < item.itemOptions.size(); i++) {
//                            if (item.itemOptions.get(i).optionTemplate.id == 159 && System.currentTimeMillis() - player.lastTimeUseKikoho > 60000) {
//                                player.lastTimeUseKikoho = System.currentTimeMillis();
//                            }
//                        }
//                    }
//                }
...
//                        }
    ...
    private void playerAttackPlayer2(Player plAtt, Player plInjure, boolean miss) {
        ...
        if (plAtt.inventory != null && plAtt.inventory.itemsBody.size() > 0 && plAtt.inventory.itemsBody.get(5) != null) {
            Item item = plAtt.inventory.itemsBody.get(5);
            if (item.itemOptions != null && item.itemOptions.size() > 0) {
                for (int i = 0; i < item.itemOptions.size(); i++) {
                    if (item.itemOptions.get(i).optionTemplate.id == 159 && System.currentTimeMillis() - plAtt.lastTimeUseKikoho > 60000 && System.currentTimeMillis() - plAtt.lastTimeUseCTX3X4 > 60000) {
                        int param = item.itemOptions.get(i).param;
                        if (plInjure.isBoss) {
                            dameHit = dameHit * param;
                            plAtt.lastTimeUseKikoho = System.currentTimeMillis();
                        }
        ...
        if (!plAtt.isBoss) {
            PlayerService.gI().sendInfoHp(plInjure);
            PlayerService.gI().sendInfoHpMpMoney(plInjure);
            Service.getInstance().Send_Info_NV(plInjure);
            if (plInjure.nPoint.hp > Integer.MAX_VALUE) {
                Service.getInstance().sendThongBao(plAtt, "HP còn lại của " + plInjure.name + " là :\n" + plInjure.nPoint.hp);
            }
        }
    }

    private void playerAttackMob2(Player plAtt, Mob mob, boolean miss, boolean dieWhenHpFull) {
        if (!mob.isDie()) {
            ...
            if (plAtt.inventory != null && plAtt.inventory.itemsBody.size() > 0 && plAtt.inventory.itemsBody.get(5) != null) {
                ...
                if (item.itemOptions != null && item.itemOptions.size() > 0) {
                    for (int i = 0; i < item.itemOptions.size(); i++) {
                        if (item.itemOptions.get(i).optionTemplate.id == 159 && System.currentTimeMillis() - plAtt.lastTimeUseKikoho > 60000) {
                            int param = item.itemOptions.get(i).param;
