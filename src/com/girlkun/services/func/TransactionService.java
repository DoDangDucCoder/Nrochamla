...
public class TransactionService implements Runnable {
        ...
        try {
            ...
            switch (action) {
                  case SEND_INVITE_TRADE:
                     Player plMapp = pl.zone.getPlayerInMap(pl.iDMark.getPlayerTradeId());
                     if (!pl.getSession().actived) {
            Service.getInstance().sendThongBao(pl, "Bạn chưa là cư dân  chính thức của Server");
            return;
        }
                case ACCEPT_TRADE:
    ...
    private void sendInviteTrade(Player plInvite, Player plReceive) {
         Player plMapp = plReceive.zone.getPlayerInMap(plReceive.iDMark.getPlayerTradeId());
                     if (!plReceive.getSession().actived) {
            Service.getInstance().sendThongBao(plInvite, "Đối phương chưa là cư dân chính thức của Server");
