...
//        player.effectSkill.timeAnTroi = timeAnTroi;
    ...
    public void sendEffectMonkey(Player player) {
     ...
     public void sethoada(Player player, long lastTimeHoaDa, int Timehoada) {
        ...
        Service.getInstance().Send_HoaDa(player);
        Service.getInstance().sendThongBao(player, "Bạn đã bị hóa đá");
        EffectSkillService.gI().sendEffectPlayer(player, player, TURN_ON_EFFECT, STONE_EFFECT);
    }
     
    public void removehoada(Player player) {
      Service.getInstance().Send_Caitrang(player);
      EffectSkillService.gI().sendEffectPlayer(player, player, TURN_OFF_EFFECT, STONE_EFFECT);
    }
}
